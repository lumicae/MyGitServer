...
<ul class="fileUl">
	...
	<li class="licon">
		...
		<a class="URL" href="${page.URL}" title="${page.URL}" target="_blank">查看</a>
		<c:if test="${page.src=='正文未检出'}">
			<a class="fileloc">无正文</a>
		</c:if>
		<c:if test="${page.src!='正文未检出'}">
			<a class="fileloc" href="/SecureInspect/preview/downSrcFile/${page.pageId}">下载</a>
		</c:if>
		...
		<span class="type">
			...
			<c:if test="${page.containFileFlag=='yes' && src != null}"><span>正文</span><span style="border-top:1px solid gray">文件</span></c:if>
			<c:if test="${page.containFileFlag=='yes' && src == null}"><span>文件</span></c:if>
			<c:if test="${page.containFileFlag=='no'}"><span>正文</span></c:if>
		</span>
		<span class="operation">
			<c:if test="${page.containFileFlag=='yes' && src != null}">
				<span><input class="handle" type="button" onclick="dealContext(this)" value="处理"></span>
				<span style="border-top:1px solid gray;"><input style="margin-top:10px;" type="button" onclick="getFileListByPageId(this, 1)" value="展开"></span>
			</c:if>
			<c:if test="${page.containFileFlag=='yes' && src == null}">
				<span><input type="button" onclick="getFileListByPageId(this, 1)" value="展开"></span>
			</c:if>
			<c:if test="${page.containFileFlag=='no'}">
				<span><input class="handle" type="button" onclick="dealContext(this)" value="处理"></span>
			</c:if>
		</span>
		<span class="kwd">
