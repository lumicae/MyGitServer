...
function dealContext(obj){
	...
	var url = "/SecureInspect/mvc/dealContext";
...
function bindSaveClk(obj){
	$(window.parent.document.body).find(".save").on("click", function(){
		...
		var url = "/SecureInspect/mvc/saveContextResult";
		...
		$.ajax({
			...
			success:function(flag){
				if(flag=="ok"){
					...
					var li = $(obj).parents("li");
					var pageId = $(li).find(".pageId").val();
					var urlp = "/SecureInspect/mvc/getPageSecureResult";
					$.ajax({
						cache:false,
						sync:false,
						type:"get",
...
function getPageList(obj, curPage){
	...
	var url = "/SecureInspect/mvc/getPageListByTaskId";
	$.ajax({
		cache:false,
		sync: false,
		type:"post",
...
function getFileList(obj, curPage){
	...
	//获取所有关键字列表，并将已检查的关键字设为选中状态
	var url = "/SecureInspect/mvc/getFileListByPageId";
	$.ajax({
		cache:false,
		sync: false,
		type:"post",
...
function loadTask(){
	 $.ajax({
		...
		cache : false,
		type : "POST",
		url : "/SecureInspect/mvc/firstLoadTask",
		dataType : "html",
		success : function(data) {
			$(".siteDiv").show();
			$(".siteDiv").html(data);
...
function loadPage(){
	$.ajax({
		...
		cache : false,
		type : "POST",
		url : "/SecureInspect/mvc/firstLoadPage",
		dataType : "html",
		success : function(data) {
			$(".siteDiv").hide();
			$(".fileDiv").hide();
...
function loadFile(){
	$.ajax({
		...
		cache : false,
		type : "POST",
		url : "/SecureInspect/mvc/firstLoadFile",
		dataType : "html",
		success : function(data) {
			$(".siteDiv").hide();
			$(".pageDiv").hide();
...
function briefSearch(){
	...
	var url = "/SecureInspect/mvc/briefSearch" + type;
...
function advSearch(){
	...
	var pageSize = $("select[name=pageSize]").val();
	var url = "/SecureInspect/mvc/advSearch" + type;
	if($(".searchSelect").val() == "task_name"){
		$("input[name=briefSrchKey]").val("task_name");
		$("input[name=briefSrchVal]").val(temp);
	}
...
function getDataByPage(curPage){
	...
	var data = null;
	switch(byPageFlag){
	case "taskId" : url = "/SecureInspect/mvc/getPageListByTaskId"; var siteId = $("." + type + "Div input[name=siteId]").val();data={"taskId": taskId, "curPage" : curPage};
		break;
	case "pageId" : url = "/SecureInspect/mvc/getFileListByPageId"; var siteId = $("." + type + "Div input[name=pageId]").val();data={"pageId": pageId, "curPage" : curPage};
		break;
	case "sitedefault" :
	case "sitebrief" : url = "/SecureInspect/mvc/briefSearchsite"; data = $("#searchForm").serialize();
		break;
	case "pagedefault" :
	case "pagebrief" : url = "/SecureInspect/mvc/briefSearchpage"; data = $("#searchForm").serialize();
		break;
	case "filedefault" :
	case "filebrief" : url = "/SecureInspect/mvc/briefSearchfile"; data = $("#searchForm").serialize();
		break;
	case "taskadv" : url = "/SecureInspect/mvc/advSearchtask"; data = $("#searchForm").serialize();
		break;
	case "pageadv" : url = "/SecureInspect/mvc/advSearchpage"; data = $("#searchForm").serialize();
		break;
	case "fileadv" : url = "/SecureInspect/mvc/advSearchfile"; data = $("#searchForm").serialize();
		break;
	}
	$.ajax({
		type : "POST",
...
function batchDelete(){
	...
	var url = "/SecureInspect/mvc/batchDelete";
	$.ajax({
		cache:false,
		sync: false,
		type:"post",
...
function batchExportReport(){
	var itemList = document.getElementsByName("itemCk");
	var url = "/SecureInspect/mvc/sendObjectArr";
	var siteArr = [];
	for(var i=0; i<itemList.length; i++){
		var site = {};
		if (itemList[i].checked){
...
function modify(obj){
	...
	var keyhtml = "";
	//获取所有关键字列表，并将已检查的关键字设为选中状态
	var url = "/SecureInspect/mvc/getSiteKeyword";
	$.ajax({
		cache:false,
		sync: false,
		type:"post",
		...
		success:function(data){
			...
			var html = "<div class='propModifySiteInfo'><input type='hidden' class='siteId' value='" + siteId +"'>" +
							 "<div><span>网站名称：</span><span class='siteName'>" + siteName + "</span></div>" + 
...
<div class="rightWrap">
<div class="navDiv">
	<a class="siteSearch" name="task" onclick="loadTask()">公众号查询</a>
...
<div class="functionDiv">
	...
	<select class="searchSelect">
		...
		<option value="site_name">公众号</option>
	</select>
...
<div class="taskDiv siteDiv listDiv"></div>
