...
public class MainBaseDAO<T>{
	 ...
	 */
	@Autowired
	@Qualifier("webJDBCTemplate")
	protected JdbcTemplate webJdbcTemplate;
	
	@Autowired
	@Qualifier("webMainNamedParameterJdbcTemplate")
	protected NamedParameterJdbcTemplate webNamedParameterJdbcTemplate;
	
	
	
	/*
	 * 数据保存方法，包括：
	 * 单数据实体保存方法——save(T entity)；
	 * 多数据实体保存方法——batchSave(Collection<T> entityCollection)；
	...
	public int save(T entity) {	
		...
		return webNamedParameterJdbcTemplate.update(sql.toString(), sps);
		/*
	...
	public int[] batchSave(Collection<T> entityCollection){
		...
		return webNamedParameterJdbcTemplate.batchUpdate(sql.toString(), batchSPS);
		/*
		SimpleJdbcInsert insertActor = new SimpleJdbcInsert(jdbcTemplateContent);
		SqlParameterSource[] batchSPS = SqlParameterSourceUtils.createBatch(entityCollection.toArray());
	...
	/*
	 * 数据删除方法，包括：
	 * 单数据实体删除方法——delete(T entity)，deleteById(Serializable id)；
	...
	public int delete(T entity){
		...
		return webNamedParameterJdbcTemplate.update(sql, sps);
	}
	...
	public int deleteById(Serializable id){
		...
		return webJdbcTemplate.update(sql, id);
	}
	...
	public int[] batchDelete(Collection<T> entityCollection){
		...
		return webNamedParameterJdbcTemplate.batchUpdate(sql, batchSPS);
	}
		...
		for(Object obj : objList){
			...
			for(Field field : fields){
				if(field.getType().equals(List.class)){
					...
					List list = webJdbcTemplate.query(sql, BeanPropertyRowMapper.newInstance(GenericsUtils.getFieldGenericType(field)));//获得子表列表
					if(!list.isEmpty())
	...
	public boolean cascadedDeleteById(Serializable id){
		...
		Object obj = webJdbcTemplate.queryForObject(sql, BeanPropertyRowMapper.newInstance(entityClass), id);	
		List<Object> objList = new Vector<Object>();
		objList.add(obj);
	...
	/*
	 * 数据更新方法，包括：
	 * 单数据实体更新方法——update(T entity)；
	 * 多数据实体更新方法——batchUpdate(Collection<T> entityCollection)；
	 * 单数据实体级联更新方法——cascadedUpdate(T entity)，cascadedSetActiveById(Serializable id, boolean active)；
	...
	*/
		...
		for(Object obj : objList){
			...
			for(Field field : fields){
				if(field.getType().equals(List.class)){
					...
					List list = webJdbcTemplate.query(sql2, BeanPropertyRowMapper.newInstance(GenericsUtils.getFieldGenericType(field)));//获得子表列表
					if(!list.isEmpty())
						tempList.addAll(list);
				}
			}
	...
	public T findActivatedById(Serializable id){
		...
		return webJdbcTemplate.queryForObject(sql, BeanPropertyRowMapper.newInstance(entityClass), id);
	}
	...
	public List<T> findAllActivated(Class<T> entityClass, Serializable order){
		...
		return webJdbcTemplate.query(sql, BeanPropertyRowMapper.newInstance(entityClass));
	}
	...
	public List<?> findByQueryForList(String sql, Class<?> elementType){
		...
		return webJdbcTemplate.queryForList(sql, elementType);
	}
	...
	public long getTotalItemsNumBySelectQuery(String sql){
		...
		if(temp[0].equalsIgnoreCase("SELECT")){
			...
			//计算符合条件的总数据条目数
			totalNum = webJdbcTemplate.queryForObject(countSql, Long.class);
		}
	...
	public boolean pagingFindBySelectQueryAndSortByRowidForSingleTable(String sql, Class<?> entityClass, PageView<T> pageView, Serializable colToSort, Serializable order){
		...
		if(temp[0].equalsIgnoreCase("SELECT") && pageView.getTotalNum() != 0){
			...
			List<?> items = webJdbcTemplate.query(querySql, BeanPropertyRowMapper.newInstance(entityClass));
	...
	public boolean pagingFindBySelectQueryAndSortByRownumForSingleTable(String sql, Class<?> entityClass, PageView<T> pageView){
		...
		long fstPara = pageView.getLimit1sdPara();
		long srdPara = pageView.getLimit2rdPara();
	...
	public T cascadedQueryById(Serializable id, boolean active, short rank, Serializable order){
		...
		Object result = webJdbcTemplate.queryForObject(sql1, BeanPropertyRowMapper.newInstance(entityClass), id);
		List<Object> objList = new Vector<Object>();
		objList.add(result);
	...
	/*
	 * 自定义SQL查询，该方法无任何类型限制、无返回值，使用时请格外留意。
	 */
	public void executeBySql(String sql){	
		webJdbcTemplate.execute(sql);
	}
